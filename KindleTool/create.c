    ...
    while((opt = getopt_long(argc, argv, "d:k:b:s:t:1:2:m:p:B:h:c:o:r:x:auUC", opts, &opt_index)) != -1)
    {
        switch(opt)
        {
            case 'd':
                ...
                {
                    strncpy(info.magic_number, "FD04", 4);
                    unsigned int num_aliased_devices = 7 + kt_with_unknown_devcodes;
                    info.devices = realloc(info.devices, (info.num_devices + num_aliased_devices) * sizeof(Device));
                    ...
                    if(kt_with_unknown_devcodes)
                        info.devices[info.num_devices++] = ValidKindlePinot_0xD6;
                    ...
                    info.devices[info.num_devices++] = KindlePaperWhite2Wifi3GAfrica;
                }
                else if(strcmp(optarg, "kindle5") == 0)
                {
                    strncpy(info.magic_number, "FD04", 4);
                    unsigned int num_aliased_devices = 16 + (kt_with_unknown_devcodes * 2);
                    info.devices = realloc(info.devices, (info.num_devices + num_aliased_devices) * sizeof(Device));
                    info.devices[info.num_devices++] = Kindle5TouchWifi;
                    info.devices[info.num_devices++] = Kindle5TouchWifi3G;
                    info.devices[info.num_devices++] = Kindle5TouchWifi3GEurope;
                ...
                else
                {
                    ...
                    else if(strcmp(optarg, "pw2ga") == 0 || strcmp(optarg, "kpw2ga") == 0)
                    {
                        info.devices[info.num_devices - 1] = KindlePaperWhite2Wifi3GAfrica;
                        strncpy(info.magic_number, "FD04", 4);
                    }

...
if [[ ! -d "openssl-1.0.1c" ]] ; then
	...
	# Can't disable dsa, pem, conf, comp, ts, ocsp, ui & stack or the MinGW TC barfs...
	./Configure mingw -DL_ENDIAN ${BASE_CFLAGS} -fno-strict-aliasing no-bf no-camellia no-cast no-cms no-des no-dh no-ec no-ecdh no-ecdsa no-idea no-gost no-jpake no-krb5 no-md2 no-md4 no-mdc2 no-rc2 no-rc4 no-rc5 no-ripemd no-srp no-whrlpool no-zlib no-ssl no-pqueue no-store no-txt_db no-modes --prefix=${TC_BUILD_DIR} --openssldir=${TC_BUILD_DIR}/etc/ssl no-shared threads

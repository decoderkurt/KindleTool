...
if [[ ! -d "openssl-1.0.1c" ]] ; then
	...
	# Can't disable dsa, pem, conf, comp, ts, ocsp, ui, stack, txt_db, modes & whrlpool or the MinGW build fails...
	./Configure mingw -DL_ENDIAN ${BASE_CFLAGS} -fno-strict-aliasing no-bf no-camellia no-cast no-cms no-des no-dh no-ec no-ecdh no-ecdsa no-idea no-gost no-jpake no-krb5 no-md2 no-md4 no-mdc2 no-rc2 no-rc4 no-rc5 no-ripemd no-srp no-zlib no-ssl no-pqueue no-store --prefix=${TC_BUILD_DIR} --openssldir=${TC_BUILD_DIR}/etc/ssl no-shared threads
